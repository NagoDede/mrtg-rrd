...
#use '/usr/lib/arm-linux-gnueabihf/perl5/5.24/RRDs'
use RRDs;
...
#Change the version number as there is perl evolution correction
#and code evolution to display in one column instead of 2
$version = '0.7.1 - custo';
#$version = '0.7';
...
#this was a first implementation
	#return @{$target->{args}} if ($target->{args} && @{$target->{args}});
#But the discussion https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=799097
#present a better way
	...
	%{$target->{options}} = ()
#The discussion https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=799097 presents a more appropriate solution
...
#Update to correct Perl update
	#if (@{$directories{$dir}{subdir}}) {
#First modification
	#if ($directories{$dir}{subdir} && @{$directories{$dir}{subdir}}) {	
#Discussion presnts https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=799097 different solution
...
#Discussion https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=799097 presents different way
...
EOF
##########################################################
#The modifications will display the graph on a single column instead of two
#This will increase de display in wordpress website
#Data: 22/09/2018
#odd computation not required as there is one column
#		my $odd;
		for my $item (@{$directories{$dir}{target}}) {
			my $itemname = $item;
			$itemname = $targets{$item}{title}
				if defined $targets{$item}{title};
#change to have all the graph in a single column
#			print "    <TR>\n" unless $odd;
#
#			print <<EOF;
...
#EOF
                       ...
                       print <<EOF;
   <TR><A HREF="$item.html">$itemname<BR>
       <IMG SRC="$item-day.$imagetype" BORDER=0 ALIGN=TOP VSPACE=10 ALT="$item">
       </A><BR CLEAR=ALL>
   </TR>
EOF
...
#As there is now a single column no need to the odd computation
#furthemore, it seems buggy as a </TR> there is two much </TR> if we have a odd pair of graph
#			print "    </TR>\n" if $odd;
#			$odd = !$odd;
		} 
#		print "    </TR>\n</TABLE>\n";
...
#the websites do not exist anymore, replace to mrtg website
#<tr>
...
<tr>
<td WIDTH=63><a ALT="MRTG"
    HREF="https://oss.oetiker.ch/mrtg/"><img
BORDER=0 SRC="http://oss.oetiker.ch/mrtg/fb-logo.png"></a></td>
</tr>
